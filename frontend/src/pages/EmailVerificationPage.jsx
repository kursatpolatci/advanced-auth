...
function EmailVerificationPage() {
    ...
    const handleChange = (index, value) => {
      ...
      if (value.length > 1) {
        ...
        const lastFillexIndex = newCode.findLastIndex((digit) => digit !== "")
        const focusIndex = lastFillexIndex < 5 ? lastFillexIndex + 1 : 5
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="flex justify-between">
          {code.map((digit, index) => (
            <input 
              ...
              className="w-12 h-12 text-center text-2xl text-white font-bold bg-gray-700 border-2 border-gray-600 rounded-lg focus:border-green-500 focus:outline-none"
            />
          ))}
        </div>
        ...
        >
          {isLoading ? "Verifyng..." : "Verify Email"}
        </motion.button>
      </form>
    </motion.div>
  )
